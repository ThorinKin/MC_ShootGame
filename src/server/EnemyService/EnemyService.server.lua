...
-- 当前版本就一个 EnemySpawner，里面一个 Middle + 一堆 location_*
local SpawnerInfo = {
...
local function spawnOneEnemy()
    ...
    enemy:SetAttribute("HomePos", centerPos)
...
local function updateEnemy(enemy: Model, dt: number)
    ...
    local attackRange    = getAttrNumber(enemy, "AttackRange",    DEFAULTS.AttackRange)
    local attackDamage   = getAttrNumber(enemy, "AttackDamage",   DEFAULTS.AttackDamage)
    local attackCooldown = getAttrNumber(enemy, "AttackCooldown", DEFAULTS.AttackCooldown)
    ...
    if targetPlayer and targetRoot and targetHumanoid then
        ...
        if distEnemyToPlayer > attackRange then
            -- 追击
            ...
            local now = time()
            if now - data.lastAttackTime >= attackCooldown then
